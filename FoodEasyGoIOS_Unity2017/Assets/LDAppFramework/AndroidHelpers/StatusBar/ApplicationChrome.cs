...
class ApplicationChrome {
	...
	/**
	 * Source: http://zehfernando.com/2015/unity-tidbits-changing-the-visibility-of-androids-navigation-and-status-bars-and-implementing-immersive-mode/
	 * Usage: 
	...
	private static void applyUIColors() {
		...
		private static void setSystemUiVisibilityInThread() {
			//DebugLogger.Log("SYSTEM FLAGS: " + systemUiVisibilityValue);
			using (var unityPlayer = new AndroidJavaClass("com.unity3d.player.UnityPlayer")) {
				using (var activity = unityPlayer.GetStatic<AndroidJavaObject>("currentActivity")) {
		...
		private static void setFlagsInThread() {
			//DebugLogger.Log("FLAGS: " + flagsValue);
			using (var unityPlayer = new AndroidJavaClass("com.unity3d.player.UnityPlayer")) {
				using (var activity = unityPlayer.GetStatic<AndroidJavaObject>("currentActivity")) {
					using (var window = activity.Call<AndroidJavaObject>("getWindow")) {
		...
		private static void applyUIColorsAndroidInThread() {
			using (var unityPlayer = new AndroidJavaClass("com.unity3d.player.UnityPlayer")) {
				using (var activity = unityPlayer.GetStatic<AndroidJavaObject>("currentActivity")) {
					using (var window = activity.Call<AndroidJavaObject>("getWindow")) {
						//DebugLogger.Log("Colors SET: " + _statusBarColor);
