...
//				UnityEngine.DebugLogger.Log( "Opening project " + filePath );
				this.projectRootPath = Path.GetDirectoryName( filePath );
				this.filePath = filePath;
			} else {
//				UnityEngine.DebugLogger.Log( "Looking for xcodeproj files in " + filePath );
...
//				_rootGroup = (PBXDictionary)_objects[ (string)_rootObject[ "mainGroup" ] ];
		...
		{
			foreach( KeyValuePair<string, XCBuildConfiguration> buildConfig in buildConfigurations ) {
//				UnityEngine.DebugLogger.Log( "ADDING HEADER PATH: " + paths + " to " + buildConfig.Key );
		...
		public PBXDictionary AddFile( string filePath, PBXGroup parent = null, string tree = "SOURCE_ROOT", bool createBuildFiles = true, bool weak = false )
		{
			...
			if( Path.IsPathRooted( filePath ) ) {
				absPath = filePath;
//				UnityEngine.DebugLogger.Log( "Is rooted: " + absPath );
			}
			else if( tree.CompareTo( "SDKROOT" ) != 0) {
				absPath = Path.Combine( Application.dataPath.Replace("Assets", ""), filePath );
//				UnityEngine.DebugLogger.Log( "RElative: " + absPath );
			}
			
			if( !( File.Exists( absPath ) || Directory.Exists( absPath ) ) && tree.CompareTo( "SDKROOT" ) != 0 ) {
//				UnityEngine.DebugLogger.Log( "Missing file: " + absPath + " > " + filePath );
				return results;
			}
...
//			UnityEngine.DebugLogger.Log( "Add file result path: " + filePath );
...
//				UnityEngine.DebugLogger.Log( "File già presente." );
				return null;
			}
...
//							UnityEngine.DebugLogger.LogError(absPath);
...
//				UnityEngine.DebugLogger.Log( "DIR: " + directory );
...
//					UnityEngine.DebugLogger.Log( "fatto" );
					continue;
				}
				
				if( recursive ) {
//					UnityEngine.DebugLogger.Log( "recursive" );
...
//				UnityEngine.DebugLogger.Log( "--> " + file + ", " + newGroup );
		...
		{
//			UnityEngine.DebugLogger.Log( "GetGroup: " + name + ", " + path + ", " + parent );
			if( string.IsNullOrEmpty( name ) )
				return null;
...
//				UnityEngine.DebugLogger.Log( "current: " + current.Value.guid + ", " + current.Value.name + ", " + current.Value.path + " - " + parent.HasChild( current.Key ) );
...
//			UnityEngine.DebugLogger.Log( "Adding libraries..." );
...
//			UnityEngine.DebugLogger.Log( "Adding frameworks..." );
...
//			UnityEngine.DebugLogger.Log( "Adding files..." );
...
//			UnityEngine.DebugLogger.Log( "Adding folders..." );
...
//			UnityEngine.DebugLogger.Log( "Adding headerpaths..." );
			foreach( string headerpath in mod.headerpaths ) {
				string absoluteHeaderPath = System.IO.Path.Combine( mod.path, headerpath );
				this.AddHeaderSearchPaths( absoluteHeaderPath );
			}
//			UnityEngine.DebugLogger.Log( "Adding librarypaths..." );
...
//			UnityEngine.DebugLogger.Log ("Adding Zip...");
...
//			UnityEngine.DebugLogger.Log( "Configure build settings..." );
			Hashtable buildSettings = mod.buildSettings;
			if( buildSettings.ContainsKey("OTHER_LDFLAGS") )
			{
//				UnityEngine.DebugLogger.Log( "    Adding other linker flags..." );
			...
			{
//				UnityEngine.DebugLogger.Log( "    GCC_ENABLE_CPP_EXCEPTIONS = " + buildSettings["GCC_ENABLE_CPP_EXCEPTIONS"] );
			...
			{
//				UnityEngine.DebugLogger.Log( "    GCC_ENABLE_OBJC_EXCEPTIONS = " + buildSettings["GCC_ENABLE_OBJC_EXCEPTIONS"] );
