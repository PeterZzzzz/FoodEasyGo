...
public class PlaceOrderPanelUniwebview : MonoBehaviour {
    ...
    public string url;
    public static GameObject webViewGameObject;

#if UNITY_IOS || UNITY_ANDROID || UNITY_WP8


    public void OpenURL () {
        url = Config.serverAPIURL + "StripePayment/order_pay_page?order_id=" + PlaceOrderPanelController.instance.orderID;

        DebugLogger.Log ("Opening uniwebview: " + url);
    ...
    void OnLoadComplete (UniWebView webView, bool success, string errorMessage) {
        if (success) {
            ...
            DebugLogger.Log ("Something wrong in webview loading: " + errorMessage);
        }
    }
    ...
    UniWebViewEdgeInsets InsetsForScreenOreitation (UniWebView webView, UniWebViewOrientation orientation) {
        ...
        if (orientation == UniWebViewOrientation.Portrait) {
            return new UniWebViewEdgeInsets (200, 50, 100, 50);
        } else {
    ...
    void Start() {
        DebugLogger.LogWarning("UniWebView only works on iOS/Android/WP8. Please switch to these platforms in Build Settings.");
    }
#endif
