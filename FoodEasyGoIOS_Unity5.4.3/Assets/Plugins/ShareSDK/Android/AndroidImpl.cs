...
namespace cn.sharesdk.unity3d
{
	...
	public class AndroidImpl : ShareSDKImpl
	{
		...
		{
			DebugLogger.Log("AndroidImpl  ===>>>  AndroidImpl" );
			try{
		...
		{
			DebugLogger.Log("AndroidImpl  ===>>>  InitSDK === " + appKey);
			if (ssdk != null) 
			{			
		...
		public override void SetPlatformConfig (Hashtable configs) 
		{
			...
			DebugLogger.Log("AndroidImpl  ===>>>  SetPlatformConfig === " + json);
			if (ssdk != null) 
			{			
		...
		{
			DebugLogger.Log("AndroidImpl  ===>>>  Authorize" );
			if (ssdk != null) 
			{
		...
		{
			DebugLogger.Log("AndroidImpl  ===>>>  ShowUser" );
			if (ssdk != null) 
			{
		...
		{
			DebugLogger.Log("AndroidImpl  ===>>>  ShareContent to one platform" );
		...
		{
			DebugLogger.Log("AndroidImpl  ===>>>  Share" );
			if (ssdk != null) 
			{
		...
		{
			DebugLogger.Log("AndroidImpl  ===>>>  OnekeyShare platform ===" + (int)platform );
			if (ssdk != null) 
			{
		...
		{
			DebugLogger.Log("AndroidImpl  ===>>>  GetFriendList" );
			if (ssdk != null) 
			{
		...
		{
			DebugLogger.Log("AndroidImpl  ===>>>  FollowFriend" );
			if (ssdk != null) 
			{
		...
		{
			DebugLogger.Log("AndroidImpl  ===>>>  GetAuthInfo" );
			if (ssdk != null) 
			{
		...
		{
			DebugLogger.Log("AndroidImpl  ===>>>  DisableSSOWhenAuthorize" );
			if (ssdk != null) 
			{
		...
		{
			DebugLogger.Log("#WARING : Do not support this feature in Android temporarily" );
		}
